            ...
            if (jsonObject.has("voices")) {
                ...
                for (int i = 0; i < voicesArray.length(); i++) {
                    ...
                    JsonResponseofGetAvailableVoices.Voice voice = new JsonResponseofGetAvailableVoices.Voice(
                            voiceObject.optString("value"),
                            voiceObject.optString("name"),
                            voiceObject.optString("language"),
                            voiceObject.optString("voiceType"),
                            voiceObject.optString("languageCode"),
                            voiceObject.optString("gender"),
                            voiceObject.optString("service"),
                            voiceObject.optString("sample")
                    );
                    voiceList.add(voice);
                }
            }
